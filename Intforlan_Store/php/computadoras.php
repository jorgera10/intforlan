...
$sentencia = $pdo->prepare("SELECT * FROM `computadora`"); //aqui
$sentencia->execute();
...
if (!$_GET) {
    ...
    echo '<meta http-equiv="refresh" content="50000000;url=computadoras.php?pages=1">'; //aqui
}

if ($_GET['pages'] < 1 || $_GET['pages'] > $totalPages) {
    //header('Location:computadoras.php?pages=1');
    echo '<meta http-equiv="refresh" content="0;url=computadoras.php?pages=1">'; //aqui
}
?>
...
$sql_articles = "SELECT * FROM computadora  LIMIT :Start, :ProductsPage"; //limit dice que del articulo tal a tal articulo //aqui
...
<nav aria-label="Page navigation d-flex justify-content-center navbar navbar-expand-lg navbar-light tm-bg-white-transparent tm-navbar">
    <ul class="pagination d-flex justify-content-center tm-bg-white-transparent "  >
        <li class=" tm-bg-white-transparent page-item <?php echo $_GET['pages'] <= 1 ? 'disabled' : '' ?> "><a class=" page-link" href="computadoras.php?pages=<?php echo $_GET['pages'] - 1 ?>">Anterior</a></li>

        <?php for ($i = 0; $i < $totalPages; $i++) { ?>
            <li class="tm-bg-white-transparent page-item <?php echo $_GET['pages'] == $i + 1 ? 'active' : '' ?>"><a class="page-link" href="computadoras.php?pages=<?php echo $i + 1 ?>"><?php echo $i + 1 ?></a></li>
        <?php } ?>

        <li class="tm-bg-white-transparent page-item <?php echo $_GET['pages'] >= $totalPages ? 'disabled' : '' ?>"><a class=" page-link" href="computadoras.php?pages=<?php echo $_GET['pages'] + 1 ?>">Siguiente</a></li>
    </ul>
</nav>
