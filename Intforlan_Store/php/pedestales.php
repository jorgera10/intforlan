...
$sentencia = $pdo->prepare("SELECT * FROM `pedestales`"); //aqui
$sentencia->execute();
...
if (!$_GET) {
    ...
    echo '<meta http-equiv="refresh" content="0;url=pedestales.php?pages=1">'; //aqui
}
...
$sql_articles = "SELECT * FROM pedestales LIMIT :Start, :ProductsPage"; //limit dice que del articulo tal a tal articulo //aqui
...
<div class="container col-12 col-xl-3 col-md-4 col-lg-3 col-xl-2  mb-5">
    <div class="card tm-bg-black-transparent contenedor">
        ...
        <a href="../_archivos/informacionPedestales.php?id=<?php echo $producto['id'] ?>"><img alt="<?php echo $producto['nombre']; ?>" title="<?php echo $producto['nombre']; ?>" src="https://github.com/intforlan/IntforlanImages/blob/master/<?php echo $producto['nombre'] ?>_I1.jpg?raw=true" data-content=<?php echo $producto['descripcion']; ?>></h3></a>
        ...
        <a href="../_archivos/informacionPedestales.php?id=<?php echo $producto['id'] ?>">
...
<nav aria-label="Page navigation d-flex justify-content-center navbar navbar-expand-lg navbar-light tm-bg-white-transparent tm-navbar">
    <ul class="pagination d-flex justify-content-center tm-bg-white-transparent "  >
        <li class=" tm-bg-white-transparent page-item <?php echo $_GET['pages'] <= 1 ? 'disabled' : '' ?> "><a class=" page-link" href="../php/pedestales.php?pages=<?php echo $_GET['pages'] - 1 ?>">Anterior</a></li>

        <?php for ($i = 0; $i < $totalPages; $i++) { ?>
            <li class="tm-bg-white-transparent page-item <?php echo $_GET['pages'] == $i + 1 ? 'active' : '' ?>"><a class="page-link" href="../php/pedestales.php?pages=<?php echo $i + 1 ?>"><?php echo $i + 1 ?></a></li>
        <?php } ?>

        <li class="tm-bg-white-transparent page-item <?php echo $_GET['pages'] >= $totalPages ? 'disabled' : '' ?>"><a class=" page-link" href="../php/pedestales.php?pages=<?php echo $_GET['pages'] + 1 ?>">Siguiente</a></li>
    </ul>
</nav>
